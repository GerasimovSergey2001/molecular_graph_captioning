...
class TrainCollater2(object):
    ...
    def __call__(self, batch):
        ...
        for p_ids, d_ids in zip(prompt_tokens.input_ids, desc_tokens.input_ids):
            ...
            # Создаем метки: промпт заменяем на -100
            combined_labels = [-100] * len(p_ids) + d_ids
            
            combined_ids = combined_ids[:self.text_max_len]
            combined_labels = combined_labels[:self.text_max_len]
            
            all_input_ids.append(torch.tensor(combined_ids))
            all_labels.append(torch.tensor(combined_labels))
        ...
        # Создаем attention_mask: 1 там, где не паддинг
        attention_mask = (input_ids != self.tokenizer.pad_token_id).long()
        
        # 4. Маска для молекулярных токенов
        mol_token_id = self.tokenizer.convert_tokens_to_ids("<mol>")
        mol_mask = (input_ids == mol_token_id)
        
        # Дополнительная обработка графа
        batch_graph.x = batch_graph.x[:, :2].clone()
        batch_graph.edge_attr = self.process_edge_attr(batch_graph.edge_attr)

        return batch_graph, input_ids, attention_mask, labels, mol_mask